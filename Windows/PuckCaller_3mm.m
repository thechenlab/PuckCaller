...
ImageSize=5000; %The image registration will output images that are not all the exact same size, because of stitching. So find_roi_stack_fun crops the images a bit. We can choose this value to be something like 0.95 * the size of the images. So e.g. for 3x3 it should be 0.95*2048*(2*(2/3)+1) = 4500. For 7x7 it is 0.95*2048*(1+6*2/3)=9728. I used 10400 previously though.
XCorrBounds=[2300,2700,2300,2700]; %This is the ROI used in channel registration
...
for puck=1:length(PuckNames) %note we are trying to run this overnight without parfor, because we can't figure out why it keeps crashing, and it only crashes for large sized images, and only on puck 8_7
    ...
    Suffix='_Stitched';
    find_roi_stack_fun_LMC(BaseName,Suffix,ImageSize,'PixelCutoff',PixelCutoffRegistration,'XCorrBounds',XCorrBounds,'RegisterColorChannels',1,'NumPar',NumPar,'BeadseqCodePath',BeadseqCodePath,'BarcodeSequence',BarcodeSequence);
